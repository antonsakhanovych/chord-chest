<script lang="ts">
	...
	import { groupTabLines } from '$lib/songs/groupTabLines';
...
<div class="overflow-x-auto font-display text-ink">
	{#each song.paragraphs as paragraph, paragraphIndex (paragraphIndex)}
		{#each groupTabLines(paragraph.lines) as run, runIndex (runIndex)}
			{#if run.kind === 'tab'}
				<div class="relative mt-4 rounded-sm bg-paper-dark py-3 pr-4 pl-5 shadow-sm">
					<span class="absolute top-2 bottom-2 left-0 w-1.5 rounded-r-sm bg-mark" aria-hidden="true"
					></span>
					<p class="mb-1 text-sm font-bold tracking-wide text-ink-faint uppercase">Tab</p>
					<pre class="overflow-x-auto whitespace-pre text-ink">{run.text}</pre>
				</div>
			{:else}
				{@const line = run.line}
